...
# (re-including one on a booted tower crashes; x-lang#515), and no collect of
# our own -- x-base.x ends its boot with one and the compiled tower leaves no
# garbage that needs a second.
cat > "$OUT" <<EOT
